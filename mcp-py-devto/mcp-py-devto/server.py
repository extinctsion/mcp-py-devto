...
async def create_article(title: str, body_markdown: str, tags: str = "", published: bool = False) -> str:
    ...
    headers = {
        ...
        "api-key": os.getenv("DEV_TO_API_KEY")
    }
    
    async with httpx.AsyncClient(timeout=10.0) as client:
        response = await client.post(f"{BASE_URL}/articles", json=article_data, headers=headers)
        response.raise_for_status()
...
#-----------------------------------above were the CREATE operations-----------------------------------
...
async def get_user_info(username: str) -> str:
    ...
    try:
        ...
        raise e
#-----------------------------------above were the READ operations-----------------------------------
...
#-----------------------------------above were the UPDATE operations-----------------------------------

#Delete opearation need to be written
...
#-----------------------------------Prompts for user interactions-----------------------------------
# Prompts to guide user interactions
@mcp.prompt()
